...
$docu_id = secureData($_GET['docu_id'],'d');

$officeCreated = secureData($_GET['officeCreated'],'d');

$refno = GetData('select referenceNo from documenttransac where docu_id='.$docu_id);
...
if($is_exist_in_documentdetails){
    mysqli_query($conn,'insert into '.$_SESSION['z_tmp_signatory'].' (os_id,indexing) 
                        select office_tag,indexing from documentdetails where referenceNo=\''.$refno.'\' ');
} else {
    echo'';
}
