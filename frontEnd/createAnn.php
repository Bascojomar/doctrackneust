            ...
            <span class="mr-2 d-none d-lg-inline text-gray-600 small">';
                    ...
                    ?>
                        ...
                        <div class="d-sm-flex align-items-center justify-content-between mb-4">
                            <h1 class="h3 mb-0 text-gray-800">Create Attachment | 
                ...
                <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#announcemodal">
                
                    <i class=""></i> Add Attachment
                </button>
                <?php
    include "modal/announceModal.php";
                    ?>
            </div>
                                ...
                                <table id="example" class="table table-striped" style="width:100%;">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-center">Add Signatory</th>
                                    ...
                                    if (isset($_SESSION['user_id'])) {
                                        ...
                                        $result = $conn->query($query);
                                                ...
                                                $sql = "SELECT * FROM attachment where is_released = 0 and user_id ='$user_id'";
                                                ...
                                                if (mysqli_num_rows($result) > 0) {
                                                    ...
                                                    while ($row = mysqli_fetch_assoc($result)) {
                                                        ...
                                                        echo'<td class="text-center"> <a type="button" title="Details" data-bs-toggle=
                                                        "modal" data-bs-target="#addoffice' . $row['attachmentID'] . '" class="btn button-s btn-sm"><i class="fa fa-tags"></i></a></td> ';
                                                        echo '<td class="text-center">';   
                                                        
                                                        $is_exist_in_documentdetails = GetData('select attachmentReferenceNo from attachmentdetails where attachmentReferenceNo=\''.$row['attachmentReferenceNo'].'\'')+0;

                                                        if($is_exist_in_documentdetails){
                                                            if($row['dateCreated'] == 0000-00-00){
                                                                echo '<form action="../backEnd/newdocuAuth.php" method="post">
                                                                <input type="text" value="'.$row['attachmentReferenceNo'].'" name="attachmentReferenceNo" hidden>
                                                        <button class="btn button-s btn-sm" name="is_released"><i class="fa fa-paper-plane"></i></button>  
                                                        </form>'; 
                                                            }
                                                        } else {
                                                        echo'';
